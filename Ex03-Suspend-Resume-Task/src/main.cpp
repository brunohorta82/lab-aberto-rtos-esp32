...
void vTaskCounter(void *pvParameters)
{
  ...
  while (1)
  {
    ...
    if (count == 10)
    {
      Serial.println("Task Led suspended");
      vTaskSuspend(taskLedHandle); //MEMORIA OCUPADA NA STACK PELA TASK LED NÃO É LIBERTADA
    }
    if (count == 15)
    {
      Serial.println("Task Led resumed");
      vTaskResume(taskLedHandle); //TASK LED INICIA NOVAMENTE
    }
